...
struct SecureForestClassifier : TreeModelBase<SecureForestParty> {
  ...
  vector<SecureForestTree> estimators;

  SecureForestClassifier(int num_classes_ = 2, float subsample_cols_ = 0.8,
                         int depth_ = 5, int min_leaf_ = 1,
  ...
  void load_estimators(vector<SecureForestTree> &_estimators) {
    estimators = _estimators;
  }

  void clear() { estimators.clear(); }

  vector<SecureForestTree> get_estimators() { return estimators; }
  ...
  void fit(vector<SecureForestParty> &parties, vector<float> &y) {
    ...
    for (int i = 0; i < row_count; i++) {
      for (int c = 0; c < num_classes; c++) {
        if (c == y[i]) {
          y_onehot_encrypted[i][c] =
              parties[active_party_id].pk.encrypt<float>(1);
        } else {
          y_onehot_encrypted[i][c] =
              parties[active_party_id].pk.encrypt<float>(0);
        }
      }
    }
    ...
    for (int i = 0; i < num_trees; i++) {
      ...
      tree.fit(&parties, &y, num_classes, &y_onehot_encrypted, min_leaf, depth,
               &prior, max_samples_ratio, mi_bound, active_party_id, n_job,
               seed);
  ...
  vector<vector<float>> predict_proba(vector<vector<float>> &x) {
    if (estimators.size() == 0) {
      vector<vector<float>> y_pred_proba(
          x.size(), vector<float>(num_classes, (1.0) / (float)(num_classes)));
      return y_pred_proba;
    } else {
