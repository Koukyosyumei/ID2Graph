...
inline void
travase_nodes_to_extract_adjacency_matrix(NodeType *node, int max_depth,
                                          SparseMatrixDOK<half> &adj_mat,
                                          half weight, int target_party_id) {
  queue<NodeType *> que;
  que.push(node);
  NodeType *temp_node;
  int temp_idxs_size;
  while (!que.empty()) {
    ...
    if (temp_node->is_leaf_flag) {
      // skip_flag = temp_node->depth <= 0 && target_party_id != -1 &&
      // temp_node->party_id != target_party_id;
      ...
      // left_skip_flag = temp_node->left->is_leaf() && target_party_id !=
      // -1 && temp_node->left->party_id != target_party_id;
      // right_skip_flag = temp_node->right->is_leaf() && target_party_id
      // != -1 && temp_node->right->party_id != target_party_id;
...
inline void travase_nodes_to_extract_adjacency_matrix_as_freerider(
    NodeType *node, int max_depth, SparseMatrixDOK<half> &adj_mat, half weight,
    int target_party_id) {
  queue<NodeType *> que;
  que.push(node);
  NodeType *temp_node;
  ...
  for (int i = 0; i < model->estimators.size(); i++) {
    if (i >= skip_round) {
      extract_adjacency_matrix_from_tree(
          ...
          pow(eta, half(i - skip_round)), target_party_id);
    }
  }
  return adj_matrix;
}
 ...
 * @return SparseMatrixDOK<half>
 */
inline SparseMatrixDOK<half> extract_adjacency_matrix_from_forest(
