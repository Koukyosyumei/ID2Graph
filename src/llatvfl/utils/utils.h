 ...
 * @return vector<T>
 */
template <typename T> inline vector<T> remove_duplicates(vector<T> &inData) {
...
template <typename T> static inline float Lerp(T v0, T v1, T t) {
  return (1 - t) * v0 + t * v1;
}
...
inline bool is_satisfied_with_lmir_bound_from_pointer(
    int num_classes, float xi, vector<float> *y,
...
inline bool is_satisfied_with_lmir_bound_with_precalculation(
    ...
    vector<float> &entire_class_cnt, vector<float> *prior,
    vector<float> &y_class_cnt_within_node) {
  float eps = 1e-15;
