 ...
 * @tparam DataType
 */
template <typename DataType> struct SparseMatrixDOK {
  size_t dim_row = 0;
  size_t dim_column = 0;
  DataType zero_val = (DataType)0;
   ...
   */
  vector<vector<DataType>> to_densematrix(DataType init_val = (DataType)0) {
  ...
  float get_nonzero_ratio() {
    ...
    */
    ...
    return float(std::accumulate(tmp_buffer.begin(), tmp_buffer.end(), 0)) /
           float(dim_row);
  }
