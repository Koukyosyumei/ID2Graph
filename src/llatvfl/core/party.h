...
struct Party {
  ...
  unordered_map<int, tuple<int, float, int>>
      lookup_table; // record_id: (feature_id, threshold, missing_value_dir)
  vector<int> temp_column_subsample;
  vector<vector<float>> temp_thresholds; // feature_id->threshold

  Party() {}
  Party(vector<vector<float>> &x_, int num_classes_, vector<int> &feature_id_,
        int &party_id_, int min_leaf_, float subsample_cols_,
        bool use_missing_value_ = false, int seed_ = 0) {
    // validate_arguments(x_, feature_id_, party_id_, min_leaf_,
    // subsample_cols_);
    x = x_;
    ...
    subsample_col_count = max(0, int(subsample_cols * float(col_count)));
  }
   ...
   */
  void validate_arguments(vector<vector<float>> &x_, vector<int> &feature_id_,
                          int &party_id_, int min_leaf_,
                          float subsample_cols_) {
    try {
      if (x_.size() == 0) {
    ...
    try {
      if (x_[0].size() != feature_id_.size()) {
        throw invalid_argument("the number of columns of x is different from "
                               "the size of feature_id");
      }
    } catch (std::exception &e) {
      std::cerr << e.what() << std::endl;
    }
  ...
  /**
   * @brief Return true if the given data goes to left at the node with the
   * specified record id
   *
   * @param record_id
   * @param xi
      ...
      try {
        if (!use_missing_value) {
          throw std::runtime_error(
              "given data contains NaN, but use_missing_value is "
              "false");
        } else {
   ...
   * @return vector<int>
   */
  vector<int> split_rows(vector<int> &idxs, int feature_opt_pos,
