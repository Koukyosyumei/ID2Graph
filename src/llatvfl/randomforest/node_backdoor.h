...
struct RandomForestBackDoorNode : Node<RandomForestBackDoorParty>
{
    vector<RandomForestBackDoorParty> *parties;
    RandomForestBackDoorNode *left, *right;
    ...
    RandomForestBackDoorNode() {}
    RandomForestBackDoorNode(vector<RandomForestBackDoorParty> *parties_, vector<float> &y_, int num_classes_,
                             vector<int> &idxs_, int depth_, vector<float> &prior_, float mi_delta_,
                             int active_party_id_ = -1, int n_job_ = 1, bool is_root_ = false)
    {
        parties = parties_;
        y = y_;
    ...
    RandomForestBackDoorNode get_left()
    {
        return *left;
    }

    RandomForestBackDoorNode get_right()
    {
        return *right;
    }
    ...
    void make_children_nodes(int best_party_id, int best_col_id, int best_threshold_id)
    {
        ...
        left = new RandomForestBackDoorNode(parties, y, num_classes, left_idxs,
                                            depth - 1, prior, mi_delta, active_party_id, n_job, false);
        if (left->is_leaf_flag == 1)
        {
            left->party_id = party_id;
        }
        right = new RandomForestBackDoorNode(parties, y, num_classes, right_idxs,
                                             depth - 1, prior, mi_delta, active_party_id, n_job, false);
