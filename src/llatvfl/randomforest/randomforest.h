...
struct RandomForestClassifier : TreeModelBase<RandomForestParty> {
  ...
  RandomForestClassifier(int num_classes_ = 2, float subsample_cols_ = 0.8,
  ...
  vector<RandomForestTree> get_estimators() { return estimators; }
  ...
  void fit(vector<RandomForestParty> &parties, vector<float> &y) {
    ...
    for (int i = 0; i < num_trees; i++) {
      ...
      tree.fit(&parties, &y, num_classes, min_leaf, depth, &prior,
               max_samples_ratio, mi_bound, active_party_id, n_job, seed);
