...
struct RandomForestNode : Node<RandomForestParty> {
  ...
  RandomForestNode(vector<RandomForestParty> *parties_, vector<float> *y_,
                   ...
                   vector<float> *prior_, float mi_bound_,
                   int active_party_id_ = -1,
   ...
   */
  RandomForestNode get_left() { return *left; }
   ...
   * @return RandomForestNode
   */
  RandomForestNode get_right() { return *right; }
  ...
  void find_split_per_party(int party_id_start, int temp_num_parties,
    ...
    for (int temp_party_id = party_id_start;
      ...
      vector<vector<pair<float, vector<float>>>> search_results =
          parties->at(temp_party_id).greedy_search_split_from_pointer(idxs, y);
