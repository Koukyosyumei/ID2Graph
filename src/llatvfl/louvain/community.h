...
struct Community {
  ...
  vector<half> in;  // in[c] = total weights within c-th commynity (\sum_{(i, j)
                    // \in c} w_(i, j))
  vector<half> tot; // tot[c]
  ...
  Community(){};
  Community(Graph *gc, int nbp, half minm, int seed = 42) {
    g = gc;
    num_nodes = g->num_nodes;

    neigh_weight.resize(num_nodes, (half)-1);
  ...
  /**
   * @brief Remove the node from its current community with which it has
   * weights_from_node_to_comm links
   *
   * @param node
   * @param comm
   * @param weights_from_node_to_comm
   */
  void remove(int node, int comm, half weights_from_node_to_comm) {
  ...
  /**
   * @brief Insert the node in comm with which it shares
   * weights_from_node_to_comm links
   *
   * @param node
   * @param comm
   * @param weights_from_node_to_comm
   */
  void insert(int node, int comm, half weights_from_node_to_comm) {
    tot[comm] += g->get_weighted_degree(node);
    in[comm] += 2 * weights_from_node_to_comm + g->get_num_selfloops(node);
   ...
   *        Note that each community consists of only ond node due to
   * contraction.
   *
   * @return half
   */
  half modularity() {
    half q(0.);
    half inv_m2 = (half)1 / (half)g->total_weight;

    for (int i = 0; i < num_nodes; i++) {
      if (tot[i] > 0)
        q += inv_m2 * ((half)in[i] - (inv_m2 * (half)tot[i] * (half)tot[i]));
      // q += (half)in[i] / m2 - ((float)tot[i] / m2) * ((float)tot[i] / m2);
    }
   ...
   * @return half
   */
  half modularity_gain(int node, int comm, float weights_from_node_to_comm,
                       float w_degree) {
    // given that node has weights_from_node_to_comm links to comm.  The formula
    // is:
    // [(In(comm)+2d(node,comm))/2m - ((tot(comm)+deg(node))/2m)^2]-
    // [In(comm)/2m - (tot(comm)/2m)^2 - (deg(node)/2m)^2]
    // where In(comm)    = number or total weights of half-links strictly inside
    // comm
    //       Tot(comm)   = number or total weights of half-links inside or
    //       outside comm (sum(degrees)) d(node,com) = number or total weights
    //       of links from node to comm deg(node)   = node degree m           =
    //       number or wegihts of all links
    // ignore const (1/2m)
    return ((half)weights_from_node_to_comm -
            (half)tot[comm] * (half)w_degree / (half)g->total_weight);
  }
  ...
  void compute_neigh_comms(unsigned int node) {
    ...
    pair<vector<unsigned int>::iterator, vector<half>::iterator>
        pointer2neigh_weight = g->get_neighbors(node);
    for (unsigned int i = 0; i < degree; i++) {
      unsigned int neigh = *(pointer2neigh_weight.first + i);
      unsigned int neigh_comm = node2community[neigh];
      half neigh_w = (g->weights.size() == 0)
                         ? (half)1.
                         : *(pointer2neigh_weight.second + i);
      if (neigh != node) {
  ...
  Graph partition2graph_binary() {
    ...
    // increment `final` if c-th commynity has one or more nodes (renumber[c] !=
    // -1)
    int final = 0;
    for (int i = 0; i < num_nodes; i++)
      if (renumber[i] != -1)
    ...
    vector<vector<int>> communities(
        final); // current node idx to original data records
    for (int node = 0; node < num_nodes; node++) {
  ...
  bool one_level() {
    ...
    int nb_moves;
    int nb_pass_done = 0;
    half new_mod = modularity();
    half cur_mod = new_mod;

    vector<int> random_indicies_to_num_nodes(num_nodes);
    iota(random_indicies_to_num_nodes.begin(),
         random_indicies_to_num_nodes.end(), 0);
    ...
    do {
      ...
      // best community
      shuffle(random_indicies_to_num_nodes.begin(),
              random_indicies_to_num_nodes.end(), gen);
      ...
      for (int node_tmp = 0; node_tmp < num_nodes; node_tmp++) {
        ...
        half w_degree = g->get_weighted_degree(node);
        ...
        half best_nblinks(0.);
        half best_increase(0.);

        for (unsigned int i = 0; i < neigh_last; i++) {
          half increase = modularity_gain(node, neigh_pos[i],
    ...
    } while ((nb_moves > 0) && (nb_pass_done < maximum_nb_pass_done));
