...
struct MPISecureBoostParty : SecureBoostParty
{
                                                                                      ...
                                                                                      use_missing_value_, seed_)
    {
        ...
        cout << "col_count is " << col_count << endl;
    ...
    vector<vector<pair<float, float>>> greedy_search_split()
    {
        ...
        cout << "size of plain idxs " << plain_gradient.size() << endl;
        cout << "size of plain idxs " << plain_hessian.size() << endl;
    ...
    {
        cout << "start encrypt search" << endl;
        // feature_id -> [(grad hess)]
        // the threshold of split_candidates_grad_hess[i][j] = temp_thresholds[i][j]
        int num_thresholds;
        if (use_missing_value)
    ...
    void run_as_passive()
    {
        ...
        while (true)
        {
            cout << "rrrr uuuu nnn" << endl;
            world.recv(0, TAG_DEPTH, current_depth);
            cout << "recv depth=" << current_depth << endl;
            cout << current_depth << " " << max_depth << endl;
            cout << "num estimators " << num_estimators << endl;
            ...
            cout << "will receive in space" << endl;
            receive_instance_space();
            cout << "recv instance space" << endl;
            send_search_results();
            world.recv(0, TAG_BEST_PARTY_ID, best_party_id);
            cout << "recv best_party_id=" << best_party_id << endl;
            if (best_party_id == party_id)
            {
                ...
                world.send(0, TAG_RECORDID, insert_lookup_table(best_col_id, best_split_col_id));
                world.send(0, TAG_BEST_INSTANCE_SPACE, split_rows(idxs, best_col_id, best_threshold_id));
            }
            cout << "end rrrr uuuu nnnn" << endl;
        }
    }
};