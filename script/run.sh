...
for s in $(seq 1 ${VALUE_Z}); do
  TRAINCMD="script/run_training.sh -s ${s} -d ${VALUE_D} -m ${VALUE_M} -p ${TEMPD} -r ${VALUE_R} -c ${VALUE_C} -a ${VALUE_A} -h ${VALUE_H} -b ${VALUE_B} -j ${VALUE_J} -n ${VALUE_N} -f ${VALUE_F} -v ${VALUE_V} -e ${VALUE_E} -l ${VALUE_L} -o ${VALUE_O} -k ${VALUE_K}"
  if [ "${FLG_X}" = "TRUE" ]; then
    TRAINCMD+=" -x"
  fi
  if [ "${FLG_Y}" = "TRUE" ]; then
