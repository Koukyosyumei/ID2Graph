...
int main(int argc, char *argv[]) {
    ...
    if (use_uniontree) {
        vector<int> result =
            extract_uniontree_from_forest<RandomForestClassifier>(&clf, 1,
                                                                  skip_round);
        ...
        if (save_adj_mat) {
            ...
            string filepath =
                folderpath + "/" + fileprefix + "_communities.out";
    ...
    for (int i = 0; i < clf.estimators.size(); i++) {
        ...
        if (save_tree_html) {
            ...
            string tree_html_filepath = folderpath + "/" + fileprefix + "_" +
                                        to_string(i) + "_tree.html";
    ...
    result_file << "Val AUC,"
                << ovr_roc_auc_score(predict_proba_val, y_true_val) << "\n";
