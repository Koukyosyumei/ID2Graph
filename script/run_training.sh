while getopts d:m:p:n:f:i:r:c:a:h:j:e:l:z:k:s:wg OPT
do
  case $OPT in
...
RUNCMD="script/build/pipeline_1_training.out -f ${VALUE_P} -p ${VALUE_S} -r ${VALUE_R} -h ${VALUE_H} -j ${VALUE_J} -c ${VALUE_C} -e ${VALUE_E} -l ${VALUE_L} -z ${VALUE_Z}"
...
if [ -e "${VALUE_P}/${VALUE_S}_communities.out" ]; then
  ...
  RUNCMD="script/build/pipeline_1_training.out -f ${VALUE_P} -p ${VALUE_S} -r ${VALUE_R} -h ${VALUE_H} -j ${VALUE_J} -c ${VALUE_C} -e ${VALUE_E}$ -l 1.0 -z ${VALUE_Z}"
  if [ "${VALUE_M}" = "xgboost" ] || [ "${VALUE_M}" = "x" ]; then
    RUNCMD+=" -a ${VALUES_A}"
  fi
  if [ "${FLG_W}" = "TRUE" ]; then
