...
RUNCMD="build/script/pipeline_1_training.out -f ${VALUE_P} -p ${VALUE_S} -r ${VALUE_R} -h ${VALUE_H} -b ${VALUE_B} -j ${VALUE_J} -c ${VALUE_C} -e ${VALUE_E} -l ${VALUE_L} -o ${VALUE_O} -z ${VALUE_Z} -w ${VALUE_W}"
...
if [ -e "${VALUE_P}/${VALUE_S}_communities.out" ]; then
  ...
  RUNCMD="build/script/pipeline_1_training.out -f ${VALUE_P} -p ${VALUE_S} -r ${VALUE_R} -h ${VALUE_H} -b ${VALUE_B} -j ${VALUE_J} -c ${VALUE_C} -e ${VALUE_E}$ -l 0.1 -o ${VALUE_O} -z ${VALUE_Z} -w ${VALUE_W}"
  if [ "${VALUE_M}" = "xgboost" ] || [ "${VALUE_M}" = "x" ] || [ "${VALUE_M}" = "secureboost" ] || [ "${VALUE_M}" = "s" ]; then
    RUNCMD+=" -a ${VALUE_A}"
  fi
  if [ "${FLG_G}" = "TRUE" ]; then
...
if [ -e "${VALUE_P}/${VALUE_S}_communities.out" ]; then
  ...
  RUNCMD="build/script/pipeline_1_training.out -f ${VALUE_P} -p ${VALUE_S} -r ${VALUE_R} -h ${VALUE_H} -b ${VALUE_B} -j ${VALUE_J} -c ${VALUE_C} -e ${VALUE_E}$ -l 1.0 -o ${VALUE_O} -z ${VALUE_Z} -w ${VALUE_W}"
  if [ "${VALUE_M}" = "xgboost" ] || [ "${VALUE_M}" = "x" ] || [ "${VALUE_M}" = "secureboost" ] || [ "${VALUE_M}" = "s" ]; then
    RUNCMD+=" -a ${VALUE_A}"
  fi
  if [ "${FLG_G}" = "TRUE" ]; then
