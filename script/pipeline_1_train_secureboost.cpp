...
int main(int argc, char *argv[]) {
  ...
  SecureBoostClassifier clf = SecureBoostClassifier(
      ...
      learning_rate, boosting_rounds, lam, const_gamma, eps, 0,
  ...
  if (use_uniontree) {
    vector<int> result = extract_uniontree_from_forest<SecureBoostClassifier>(
        &clf, 1, completely_secure_round);
    ...
    start = chrono::system_clock::now();
    SparseMatrixDOK<half> adj_matrix = extract_adjacency_matrix_from_forest(
        &clf, is_freerider, 1, completely_secure_round, (half)eta);
